...
const MosaicCard = ({ mosaic }: { mosaic: MosaicPost }) => {
  ...
  const handleCardClick = () => {
    router.push(`/mosaic?id=${mosaic.id}`);
  };
  ...
  return (
    <motion.div
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      className="rounded-lg overflow-hidden bg-black/20 backdrop-blur-sm border border-purple-500/10 hover:border-purple-500/20 transition-all group cursor-pointer"
